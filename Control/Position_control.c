...
//        OriginalVelZ.value = Limits_data(  PID_Control(&PID_ParaInfo.VelZ,&OriginalVelZ,OriginalPosZ.value,RT_Info.Height_V,0.005,1) ,2,-2);
        ...
        if(RockerControl.ZaxisPos ==0)
        {
            ...
            if(TgtHeight < Target_Info.Height && FlightControl.LaunchFlag == true){
                ...
                if(  ( ( RT_Info.VioPosition_Z < 0.8f* RT_Info.Height )
                        || ( RT_Info.VioPosition_Z > 1.2f* RT_Info.Height ) )
            ...
            if(controlCnt ==2)
            {
                    //外环单P控制器
                    float heightErro = TgtHeight - RT_Info.Height;
                    OriginalPosZ.value = Limits_data( PID_ParaInfo.PosZ.Kp * heightErro ,1.5f ,-1.5f);  //±1m/s的目标速度
            }
                //速度环PID控制器
...
//                Target_Info.Roll = Target_Info.RemoteRoll;
            ...
            case 2:
                                ...
                                Sensor_Info.Raspberry_Xaxis!=0 && Sensor_Info.Raspberry_Yaxis!=0)
                {
                        ...
                        Target_Info.DesiredAccelerationX =  Limits_data( PID_Control(&PID_ParaInfo.FlowVelX,&OriginalVelX,OriginalPosX.value,
                                                                                             RT_Info.FlowX_V,0.005f,3) , 6 , -6 );

                        OriginalPosY.value = - PID_ParaInfo.PosY.Kp * RT_Info.PointY;

                        Target_Info.DesiredAccelerationY  = Limits_data( PID_Control(&PID_ParaInfo.FlowVelY,&OriginalVelY,OriginalPosY.value,
                                                                                                RT_Info.FlowY_V,0.005f,3) , 6 , -6);
            ...
            /************************ 光流定点 ************************/
                ...
                {
                    if(  (cos(RT_Info.Pitch/57.3f)*cos(RT_Info.Roll/57.3f))>=0.99f  &&  //实时没有滚转角和俯仰角
                                        sqrt(RT_Info.FlowX_V*RT_Info.FlowX_V +RT_Info.FlowY_V *RT_Info.FlowY_V ) <=0.1f //速度<=0.1
                                                &&  FlightControl.poshold == 1 )
                    {
...
//                                                                                                                       RT_Info.VioVel_Y,0.005f,1) , 6 , -6 );
                    ...
                    ComputeDesiredAttitude(DesiredAcceleration,0);
                        ...
                        RT_Info.VioPosition_X!=0 && RT_Info.VioPosition_Y!=0 && Flight_Remote_Control.FlightControlMode == VIOPosHold)
                {
                    if(  (cos(RT_Info.Pitch/57.3f)*cos(RT_Info.Roll/57.3f))>=0.98f  &&
                                        sqrt(RT_Info.FlowX_V*RT_Info.FlowX_V +RT_Info.FlowY_V *RT_Info.FlowY_V ) <=0.2f
                                                &&  FlightControl.poshold == 1 )
                    {
...
//                {
//                    if(Sensor_Info.LineAngel <= 2 && Sensor_Info.LineAngel >= -2 )
//                    {
//                        if(RT_Info.PointY <= 0.03 && RT_Info.PointY >= -0.03)
//                        {
//                            RT_Info.PointY = 0;
//                        }
//
//                        OriginalPosX.value =  PID_ParaInfo.PosX.Kp * (-0.01f);
//                        OriginalPosY.value = - PID_ParaInfo.PosY.Kp * RT_Info.PointY;
//
//                        Target_Info.DesiredAccelerationX =  Limits_data( PID_Control(&PID_ParaInfo.FlowVelX,&OriginalVelX,OriginalPosX.value,
//                                                                                                                     RT_Info.FlowX_V,0.005f,3) , 6 , -6 );
...
//                break;
            ...
            case 6:
                if(RockerControl.XaxisPos == 0  && RockerControl.YaxisPos == 0 )
                {
                    if(Sensor_Info.isHealth == 1 && Sensor_Info.isCentre == 0)
                    {
                        if(  cos(RT_Info.Pitch/57.3f)>=0.99f  &&  //实时没有俯仰角
                                        RT_Info.FlowX_V <= 0.1f //速度<=0.1
                                                &&  FlightControl.poshold_C == 1 )
                        {
                                    Target_Info.PositionX = RT_Info.FlowX;
                                    FlightControl.setTargetPos_C = 1 ;
                                    FlightControl.poshold_C = 0 ;
                        }
                        else
                        {
                            if(FlightControl.setTargetPos_C ==1  && RockerControl.Navigation == 0)
                            {
                                    float PosXerr =  Target_Info.PositionX -  RT_Info.FlowX;
                                    OriginalFlowX.value = Limits_data( PID_ParaInfo.FlowX.Kp * PosXerr,0.6f,-0.6f);
                            }
                        ...
                        //OriginalPosX.value =  PID_ParaInfo.PosX.Kp * (-0.0f);
                        OriginalPosY.value = PID_ParaInfo.PosY.Kp * RT_Info.PointY;
                    ...
                    else if (Sensor_Info.isHealth == 1 && Sensor_Info.isCentre == 1)
                    {
                        OriginalPosX.value =  PID_ParaInfo.PosX.Kp * (60.0f - RT_Info.PointX);
                        OriginalPosY.value = PID_ParaInfo.PosY.Kp * RT_Info.PointY;
                        ...
                        if (RT_Info.PointX >= 65.0f || RT_Info.PointX <= 55.0f)
                        {
                            FlightControl.RollStart = 1;
                        }
                        else
                        {
                            FlightControl.RollStart = 0;
                        }
                    }
                ...
                else
                {
                        ...
                        OriginalFlowVelY.iOut =0 ;
                        FlightControl.poshold_C = 1;
                        FlightControl.setTargetPos_C = 0;
                }
                break;
            default:
                break;
...
*/
    ...
    Target_Info.Roll  = ConstrainFloat(atan2(R_W_B[7],R_W_B[8]), -0.3f, 0.3f) *57.3f;
    Target_Info.Pitch = - ConstrainFloat(-atan2(R_W_B[6],sqrtf(R_W_B[7]*R_W_B[7] + R_W_B[8]*R_W_B[8])), -0.3f, 0.3f) *57.3f;
