...
void Attitude_control(float PitchCalibration,float RollCalibration)
{
    ...
    #endif
    #ifdef VIO_Attitude
        if(Fly_Mode == Data_Follow)
                RT_Yaw = RT_Info.VioYaw;
        else if(Fly_Mode == Data_Flow || Fly_Mode == Data_ToCentre)
              RT_Yaw = RT_Info.Yaw;
    #endif

    if(RockerControl.Navigation == 0)
    {
//        if(Fly_Mode == Data_Line && (Sensor_Info.LineAngel > 2 || Sensor_Info.LineAngel < -2)
//                    && (cos(RT_Info.Pitch/57.3f)*cos(RT_Info.Roll/57.3f))>=0.9f  &&  //实时没有滚转角和俯仰角
//                    sqrt(RT_Info.FlowX_V*RT_Info.FlowX_V +RT_Info.FlowY_V *RT_Info.FlowY_V ) <=0.1f)
//        {
//
//            yawErro = Sensor_Info.LineAngel;
//
//            OriginalYaw.value = PID_ParaInfo.Yaw.Kp * yawErro;
//            UAVThrust.YawThrust = Limits_data( (PID_Control(&PID_ParaInfo.YawRate,&OriginalYawRate, OriginalYaw.value ,RT_Info.rateYaw,0.002,100)) * Inertia_Wz , 0.1f ,-0.1f)
...
//
//        }
        if(Fly_Mode == Data_ToCentre && Sensor_Info.isHealth == 0
                    && (cos(RT_Info.Pitch/57.3f)*cos(RT_Info.Roll/57.3f))>=0.9f  &&  //实时没有滚转角和俯仰角
                    sqrt(RT_Info.FlowX_V*RT_Info.FlowX_V +RT_Info.FlowY_V *RT_Info.FlowY_V ) <=0.1f)
        {
        ...
        else if(Fly_Mode == Data_ToCentre
                && Sensor_Info.isHealth == 1 && Sensor_Info.isCentre == 1
    ...
    else
    {
        ...
        UAVThrust.YawThrust = Limits_data( (PID_Control(&PID_ParaInfo.YawRate,&OriginalYawRate, GyroZErro,RT_Info.rateYaw,0.002,100)) * Inertia_Wz , 0.1f ,-0.1f)
                            + Limits_data( RT_Info.ratePitch * (Inertia_Wy * RT_Info.rateRoll) - (Inertia_Wx * RT_Info.ratePitch) * RT_Info.rateRoll ,  0.02f ,-0.02f)
                                                            //角速度前馈
                                                            + Limits_data( Inertia_Wz * GyrozFeedforwardLpf ,  0.02f ,-0.02f);
        YawHover =1;
...
//    /* Tmotor F40 power mode */
//     M1 =  0.097f * f1 + 0.24855f;
//     M2 =  0.097f * f2 + 0.24855f;
//     M3 =  0.097f * f3 + 0.24855f;
//     M4 =  0.097f * f4 + 0.24855f;
